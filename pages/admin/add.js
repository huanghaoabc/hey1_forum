...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.subtype == 21){
      
      cir.id = 27
      cir.title = '抖音新动态'
      tit='抖音新动态';
    }else if(options.subtype == 22){
      cir.id = 28
      cir.title = '快手新动态'
      tit='快手新动态';
    }else if (options.subtype == 23)
    {
      cir.id = 29
      cir.title = '最新玩法'
      tit='最新玩法';
    }else if (options.subtype == 24)
    {
      cir.id = 30
      cir.title = '掘金抖音'
      tit='掘金抖音';
  ...
  chooseImg: function(e){
    ...
    wx.chooseImage({
      ...
      success(res) {
        ...
        console.log("eee2",tempFilePaths);
        weigh = weigh + 10
        let row = {
          type: 2,
  ...
  subData: function(res = ''){
    ...
    if(this.data.subType == 21 || this.data.subType == 22 || this.data.subType == 23)
    {
      postData.kuaishouinfo = this.data.sendDYdt
    }
    if(this.data.subType == 24 || this.data.subType == 25)
    {
      postData.kuaishouinfo = this.data.sendinfo;
    }
    /**/
  ...
  {
    let g = this.data.sendinfo;
    let b = this.data.sendDYdt;
    g.platform_type= this.data.platformArr[e.detail.value];
    b.pingtai = this.data.platformArr[e.detail.value]
    this.setData({
      b,
      g,
  ...
  },addTypeFn:function (e)
  {
    let g = this.data.sendDYdt;
    g.platformType= this.data.addTypeArr[e.detail.value];
    this.setData({
      g,
      addTypeArrIndex:e.detail.value
     })
     console.log("类型:",g.platformType,g)
  }
